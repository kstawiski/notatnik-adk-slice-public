...
- Pre-warm the service by loading `/healthz` and `/cases` before recording.
...
| 1:14-1:26 | ADK Trace, then `/healthz` | Show tool calls in the trace briefly, then switch to `/healthz`. Point to `"model": "gemini-3.1-flash-lite"`, `"location": "global"`, and `"cases"`. | "The service runs on Cloud Run with FastAPI. The health check shows the pinned Gemini model, project, global location, and synthetic case count." |
...
- Do not add decorative overlays that obscure metrics, the discrepancy line, or `/healthz`.
